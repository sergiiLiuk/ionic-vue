...
function MapboxglSpiderifier(map, userOptions) {
  var util = {
    ...
    options = {
      ...
      circleFootSeparation: 30, // related to circumference of circle
      spiralFootSeparation: 38, // related to size of spiral (experiment!)
      spiralLengthStart: 60, // ditto
  ...
  let spiderifiedStatus = false
  this.isSpiderified = isSpiderified

  // Private:
  function spiderfy(latLng, features) {
    spiderifiedStatus = true
  ...
  function unspiderfy() {
    spiderifiedStatus = false
    util.each(previousSpiderLegs.reverse(), function(spiderLeg, index) {
      if (options.animate) {
        spiderLeg.elements.container.style['transitionDelay'] =
          (options.animationSpeed / 1000 / previousSpiderLegs.length) * index + 's'
  ...
  function isSpiderified() {
    return spiderifiedStatus
  }
  ...
  function generateSpiralParams(count, centerPt) {
    ...
    return util.mapTimes(count, function(index) {
      ...
      pt = {
        ...
        angle: angle,
        legLength: legLength - lengthError,
        index: index
      }
  ...
  function generateCircleParams(count, centerPt) {
    ...
    return util.mapTimes(count, function(index) {
      ...
      return {
        x: legLength * Math.cos(angle) + legError,
        y: legLength * Math.sin(angle),
        angle: angle,
        legLength: legLength - lengthError,
        index: index
  ...
  function createMarkerElements(spiderLegParam) {
    ...
    spiderLegParam.legLength = spiderLegParam.legLength
